...
export default function LoginForm({ onSuccess, onChangeModal }: LoginFormProps) {
  ...
  const handleChangeModal = (modalType: 'login' | 'signup' | 'forgot') => {
    resetErrors() // Reset errors when switching forms
    ...
    <div className="flex flex-col px-4 sm:px-6 md:px-10 pb-6 md:pb-10 space-y-4 md:space-y-7">
      ...
      <form action={handleSubmit}>
        <div className="space-y-2">
          <label className="block text-xs md:text-sm font-semibold">Email Address</label>
          <Input
            type="email"
            name="email"
        ...
        <div className="space-y-2 mt-4 md:mt-7">
          <label className="block text-xs md:text-sm font-semibold">Password</label>
        ...
        <div className="flex justify-between items-center mt-4 md:mt-7">
          <div className="flex items-center space-x-2 md:space-x-3">
            <div
              className={`w-3 h-3 md:w-4 md:h-4 rounded flex items-center justify-center cursor-pointer ${rememberMe ? 'bg-gradient-to-b from-[#03268B] to-[#000212]' : 'border border-[#3772FE80]'}`}
          ...
          <button
            type="button"
            onClick={() => handleChangeModal('forgot')}
            disabled={isAnyLoading}
            className="text-xs md:text-sm underline cursor-pointer disabled:opacity-50"
          >
            Forgot Password?
          </button>
        </div>
        ...
        >
          <span className="text-sm md:text-base">{loadingEmail ? 'Logging in...' : 'Log In'}</span>
        </button>
      </form>
